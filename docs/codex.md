...
| [`dbtalk-database`](../plugins/dbtalk/skills/database/SKILL.md) | SQLite 与 MySQL 间 JSONL 导出/导入，以及 `insert` / `upsert` 数据传输。 | `dbtalk database` | [数据库传输手册](database.md) |

两个 skill 只在其职责范围内选择命令：原生 MySQL SQL 备份与还原使用 `dbtalk-mysql`；跨库数据传输使用 `dbtalk-database`。它们不会替代 CLI 的参数校验或配置加载。
