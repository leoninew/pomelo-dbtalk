...
| --- | --- | --- | --- |
| [`dbtalk-mysql`](../plugins/dbtalk/skills/mysql/SKILL.md) | MySQL database、backup、dump、restore、导入 `.sql`，或配置 `mysqldump` / `mysqlrestore`。 | `dbtalk mysql` | [MySQL 手册](mysql.md) |
| [`dbtalk-postgres`](../plugins/dbtalk/skills/postgres/SKILL.md) | PostgreSQL database、单库逻辑备份、恢复 custom archive 或配置 Docker client image。 | `dbtalk postgres` | [PostgreSQL 手册](postgres.md) |
