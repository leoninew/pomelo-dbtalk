...
- 本机端口唯一映射到运行中的 Docker MySQL 容器时，dump/restore 共享容器发现逻辑并直接使用该容器的默认 Unix socket；没有唯一映射容器时才使用本机客户端或 Docker fallback。
...
5. 生命周期日志沿用现有标准库 logging 配置和 `dbtalk` logger，不引入 JSON Lines、独立日志文件或外部 logging 依赖。日志字段只使用非敏感的路径、阶段、耗时和字节数。
...
3. 接通 restore 的独立目标库和 DDL 安全边界。
   ...
   - DDL 扫描通过后执行只读目标库存在性探测，再进入实际 restore。local client、mapped container 和 Docker fallback 的 restore command 都使用 `--database TARGET`；mapped container 的预检和导入均通过现有容器的 Unix socket，不经过 `host.docker.internal`。

4. 增加 dump/restore 生命周期日志和可测量进度。
   - 在 dump 和 restore 的公共操作边界使用单调时钟计算 `elapsed_ms`，统一输出`mysql dump|restore started/progress/completed/failed`；完成事件必须包含最终 `bytes`，失败事件包含阶段、耗时和安全错误摘要。
...
5. 更新文档、skill 和自动化测试。
   ...
   - 在 `tests/test_mysql.py` 或新增 `tests/test_mysql_logging.py` 覆盖 restore 目标优先级、目标库探测、`USE` 重写、所有数据库 DDL 拒绝、原始输入不变、mapped container 与 Docker fallback 的路径隔离、日志字段/事件顺序和敏感信息脱敏。
...
2. 运行定向单元测试，覆盖：
   ...
   - `started`、`progress`、`completed`、`failed` 的 elapsed_ms、bytes、阶段和脱敏字段。
